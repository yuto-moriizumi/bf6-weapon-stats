...
function parseWeaponsFile(filePath: string): WeaponData[] {
  ...
  if (currentWeapon && currentWeapon.name && currentWeapon.category === "SMG") {
    weapons.push(currentWeapon as WeaponData);
  }
...
async function main() {
  ...
  console.log(`Parsed ${weaponsData.length} SMG weapons from weapons.txt`);
  ...
  for (const weaponData of weaponsData) {
    ...
    if (existingWeapon) {
      ...
      console.log(
        `  Weapon already exists: ${weapon.name} - Adding damage data only`,
      );
    } else {
  ...
  console.log("SMG import completed successfully!");
}
