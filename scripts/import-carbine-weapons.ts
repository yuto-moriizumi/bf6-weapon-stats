...
function parseWeaponsFile(filePath: string): WeaponData[] {
  ...
  return weapons.filter(w => w.category === 'Carbine');
}
...
async function main() {
  ...
  console.log(`Parsed ${weaponsData.length} Carbine weapons from weapons.txt`);
  ...
  for (const weaponData of weaponsData) {
    ...
    if (existingWeapon) {
      console.log(`  Weapon ${weaponData.name} already exists. Adding/updating damage data only.`);
      ...
      if (weaponData.damages.length > 0) {
        console.log(`  Added/updated ${weaponData.damages.length} damage entries`);
      }
    } else {
  ...
  console.log('Carbine import completed successfully!');
}
